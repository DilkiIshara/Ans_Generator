...
arr = arr_new = None             # 0-x1 1-Y1 2-x2 3-y2
...
numberOf_Horizontal_new = numberOf_Vertical_new =  numberOf_Graph_new  = 0
graphCrossOrigin = False
ratio_Y_Axis_Value = ratio_X_Axis_Value = 1
found_X = found_Y = passfromTem_Y = passfromTem_X = generateUsingOCR = False
src = src2 = MofologyImg = graphType = srcTemplate = temSrc = cdstP_X_Y = resized_img = resized_img_New = resized_img_2 = None
...
ra_X_1 =  ra_X_2 = ra_Y_1 = ra_Y_2 = 1
hasTwoValuefor_XAxisRatio = hasTwoValuefor_YAxisRatio = check_X = check_X_usingTemplateMatching = check_Y = check_Y_usingTemplateMatching = False
hasOnly_One_Value = True

def getEqationByUsingCoordinate(): 
    global generateUsingOCR
...
def getQuadraticGraphCoodinates():
    global graphType, cdstP2, MofologyImg, cdstP_Linear
    ...
    cv.line(cdstP_Linear, (arr[negativeIndex][0], arr[negativeIndex][1]), (arr[negativeIndex][2], arr[negativeIndex][3]), (5,0,255), 2, cv.LINE_AA)
    cv.line(cdstP_Linear, (arr[positiveIndex][0], arr[positiveIndex][1]), (arr[positiveIndex][2], arr[positiveIndex][3]), (128, 0, 128), 2, cv.LINE_AA)
    ...
    # /*
       # /* This point we can check graph is symmetric on y axis */
       # 
       # 
    ...
    for i in range(sx - 5 , sx + 5):
        for j in range(minMaxY - 5, minMaxY +5):
            if(i > 0 and i < width and j > 0 and j < height ): 
                cdstP2[j,i] = (0, 255, 255)
                cdstP2[j,i] = (0, 255, 255)
                MofologyImg_2[j,i] = (0, 255 , 255)
    
    if ((quadraticType == "min") and (minMaxY > origin_Y - 5)) or ((quadraticType == "max") and (minMaxY < origin_Y + 5)) :
        hasRealRoots = True
    ...
    if totalCount > 3:
        ...
        print(" 1. Point on Graph ---> (" + str(int(round(x1))) + " , " + str(int(round(y1))) +")")
        print(" 2. Point on Graph ---> (" + str(int(round(x2))) + " , " + str(int(round(y2))) +")")
        print(" 3. Point on Graph ---> (" + str(int(round(x3))) + " , " + str(int(round(y3))) +")")
...
def draw_X_Axis():
    ...
    # print(" Pixcels  of X Axis  : " + str(pixcelForTicMark_X))
    

    sameLine = 0
    for h in range(0, noOfLines):
        l = X_arr[h][1] 
        if l == maxlength_X  or l >= maxlength_X - (pixcelForTicMark_X/2): 
            sameLine = sameLine + 1
    ...
    if (sameLine > 1): 
        ...
        # print(" Identify X using highest Length")
        found_X = True
        return
...
def find_X_Axis():
    ...
    for h in range(0, noOfLines):
        X_axis_Y_cordinate = arr[h][1]
        length_Of_X_Axis = X_arr[h][1]   
        if ((X_axis_Y_cordinate <= (min_XAxis_Ycordinate + half_pixcelForTicMark_Y + (half_pixcelForTicMark_Y/2))) and (X_axis_Y_cordinate >= (min_XAxis_Ycordinate-half_pixcelForTicMark_Y - (half_pixcelForTicMark_Y/2)))) :      
    ...
    if (count > 1): 
        ...
        for h in range(0, noOfLines):
            X_axis_Y_cordinate = arr[h][1]
            length_Of_X_Axis = X_arr[h][1]   
            # not same as Y axis Identification
            if ((X_axis_Y_cordinate <= (min_XAxis_Ycordinate + half_pixcelForTicMark_Y + (half_pixcelForTicMark_Y/4) )) and (X_axis_Y_cordinate >= (min_XAxis_Ycordinate - half_pixcelForTicMark_Y ) - (half_pixcelForTicMark_Y/4))) :   
                if ((length_Of_X_Axis >= maxlength_X*0.8)) :  
                    X_axis_cordinate = h 
                    con = con + 1
                    # print(" Check +++")
        if con == 0:
            for h in range(0, noOfLines):
                ...
                length_Of_X_Axis = X_arr[h][1]   
                if ((X_axis_Y_cordinate <= (min_XAxis_Ycordinate + half_pixcelForTicMark_Y + (half_pixcelForTicMark_Y/2))) and (X_axis_Y_cordinate >= (min_XAxis_Ycordinate-half_pixcelForTicMark_Y - (half_pixcelForTicMark_Y/2)))) :   
                    if ((length_Of_X_Axis == maxlength_X  or (length_Of_X_Axis > (maxlength_X-(maxlength_X*0.2))))) :  
                        X_axis_cordinate = h 
                        con = con + 1
                    # print(" check ++++")
        ...
        if con == 1:
            ...
            for h in range(0, noOfLines):
                ...
                length_Of_X_Axis = X_arr[h][1]   
                if ((X_axis_Y_cordinate <= (min_XAxis_Ycordinate + half_pixcelForTicMark_Y + (half_pixcelForTicMark_Y/2))) and (X_axis_Y_cordinate >= (min_XAxis_Ycordinate-half_pixcelForTicMark_Y - (half_pixcelForTicMark_Y/2)))) :   
                    if ((length_Of_X_Axis == maxlength_X  or (length_Of_X_Axis > (maxlength_X-(maxlength_X*0.2)))) and (min_XAxis_Ycordinate > X_axis_Y_cordinate)) :  
                        X_axis_cordinate = h 
                        if passfromTem_X == True:
                            return True
...
def draw_Y_Axis():
    ...
    global pixcelForTicMark_Y, found_Y, passfromTem_Y, samLine_Y
    ...
    for h in range(0, noOfLines):
        l = Y_arr[h][1] 
        if l == maxlength_Y  or l >= maxlength_Y - (pixcelForTicMark_Y/2) : 
            sameLine = sameLine + 1
    ...
    if (sameLine > 1):
        ...
        # print(" Identify y using highest Length")
        found_Y = True
        return
...
def find_Y_Axis():
    global maxlength_Y
    global Y_axis_cordinate
    global pixcelForTicMark_Y, pixcelForTicMark_X 
    ...
    for h in range(0, noOfLines):
        Y_axis_X_cordinate = arr[h][0]
        length_Of_Y_Axis = Y_arr[h][1]    
        if ((Y_axis_X_cordinate <= (min_YAxis_Xcordinate + half_pixcelForTicMark_X + (half_pixcelForTicMark_X/2) )) and (Y_axis_X_cordinate >= (min_YAxis_Xcordinate - half_pixcelForTicMark_X - (half_pixcelForTicMark_X/2))) and (Y_axis_X_cordinate != 0)) :         
    ...
    if (count > 1):
        con = 0
        for h in range(0, noOfLines):
            Y_axis_X_cordinate = Y_arr[h][0] 
            length_Of_Y_Axis = Y_arr[h][1]    
            if ((Y_axis_X_cordinate <= (min_YAxis_Xcordinate + (half_pixcelForTicMark_X) + (half_pixcelForTicMark_X/3))) and (Y_axis_X_cordinate >= (min_YAxis_Xcordinate - (half_pixcelForTicMark_X) - (half_pixcelForTicMark_X/3)))) :  
                if (length_Of_Y_Axis >= maxlength_Y*0.9) : 
                    # print(" Check +++++++++++++++")
                    Y_axis_cordinate = h
                    con = con + 1
        ...
        if con == 0:
            for h in range(0, noOfLines):
                Y_axis_X_cordinate = Y_arr[h][0] 
                length_Of_Y_Axis = Y_arr[h][1]    
                if ((Y_axis_X_cordinate <= (min_YAxis_Xcordinate + half_pixcelForTicMark_X + (half_pixcelForTicMark_X/2))) and (Y_axis_X_cordinate >= (min_YAxis_Xcordinate - half_pixcelForTicMark_X - (half_pixcelForTicMark_X/2)))) :  
                    if ((length_Of_Y_Axis == maxlength_Y or (length_Of_Y_Axis > maxlength_Y - (maxlength_Y * 0.2)))) : 
                        Y_axis_cordinate = h
                        con = con + 1
...
def draw_Graph():
    ...
    cv.line(cdstP_Linear, (arr[graph_cordinate][0], arr[graph_cordinate][1]), (arr[graph_cordinate][2], arr[graph_cordinate][3]), (0,252,0), 2, cv.LINE_AA)
...
def identifyIntersection():
    ...
    # print(" Linear Graph Go through Origin")
...
def identifyTicMarks_X_Axis():
    ...
    # Create Array
    X_Axis_Intersections = np.arange(noOfLines)
    noOfPixcels = np.arange(noOfLines)
    noOfPixcels_Index = 0
    ...
    modify_array_sort =  np.arange(noOfLines) 
    ...
    if new_pixcel_array_Index > 1:
       ...
       # print(" Tic X  : = 1"  + str(pixcelForTicMark_X))
    else :
...
def identifyTicMarks_Y_Axis():
    ...
    # Create Array
    Y_Axis_Intersections = np.arange(noOfLines)
    noOfPixcels = np.arange(noOfLines)
    noOfPixcels_Index = 0
...
def identifyRatio():
    ...
    y_Axis_ratio = np.arange(30) # ratio of number of Y axis 
...
def indentify_Y_UsingValues():
    global Y_axis_cordinate, X_axis_cordinate, found_X, maxlength_Y, textCoordinate, found_Y, pixcelForTicMark_X, pixcelForTicMark_Y
...
def indentify_X_Axis_UsingValues():
    global Y_axis_cordinate, X_axis_cordinate, found_X, maxlength_X, numberOfDigitValue, pixcelForTicMark_Y
...
def generateEquationLinearGraph():
    ...
    if (totalCount > 2):
        ...
        print(" 1. Pont on Graph ---> (" + str(int(round(x1))) + " , " + str(int(round(y1))) +")")
        print(" 2. Pont on Graph ---> (" + str(int(round(x2))) + " , " + str(int(round(y2))) +")")

        print(" Equation ----> " + " Y = " + str(m) + "X + "+ str(c))
 

def templateMatching():
    global srcTemplate, numberOf_Xaxis, numberOf_Yaxis, tem_y_arr, tem_x_arr, tem_Matching_Coordinates, xIndex , yIndex, temSrc, xy_arr, hasOnly_One_Value
    ...
    templates = [[0] * 2 for i in range(35)]

    # templates[0][0] = "tem2.5.png"
    # templates[0][1] = 2

    # templates[1][0] = "tem1.2.png"
    # templates[1][1] = 1
    ...
    # templates[24][1] = 4
    # templates[25][0] = "tem3.4.png"
    # templates[25][1] = 3

    # templates[26][0] = "tem4.4.png"
    # templates[26][1] = 4
    # templates[27][0] = "tem2.6.png"
    # templates[27][1] = 2
    # templates[28][0] = "tem6.3.png"
    # templates[28][1] = 6
    # templates[29][0] = "tem8.3.png"
    # templates[29][1] = 8
    # templates[30][0] = "tem2.7.png"
    # templates[30][1] = 2
    # templates[31][0] = "tem6.4.png"
    # templates[31][1] = 6
    # templates[32][0] = "tem8.4.png"
    # templates[32][1] = 8
    # templates[33][0] = "tem8.4.png"
    # templates[33][1] = 8
    # templates[34][0] = "tem8.4.png"
    # templates[34][1] = 8
    # # templates[33][0] = "tem2.8.png"
    # # templates[33][1] = 2
    # # templates[34][0] = "tem3.4.png"
    # # templates[34][1] = 3
    ...
    for i in range(1, 35):
        ...
        for pt in zip(*loc[::-1]): 
            ...
            cv.rectangle(src, pt, (pt[0] + w, pt[1] + h), (0, 255, 0), 2)
            haveDuplicate = False
            ...
            for k in range(0,30):
                ...
                if cur_x != 0 and ((xy_arr[k][1]) != 0): 
                    if (cur_x >= x - 10 and cur_x <= x + 10 ) and (y >= (xy_arr[k][1] - 10) and y <= (xy_arr[k][1] + 10)):
                        haveDuplicate = True
            if haveDuplicate != True:
                if xy_Index < 30:
                    xy_arr[xy_Index][0]  = x
                    xy_arr[xy_Index][1]  = y
                    xy_arr[xy_Index][2]  = number
                    xy_Index = xy_Index +1 
    ...
    if tem_Matching_Coordinates_Index == 0: 
        for i in range(1, 35):
            curretTemplate = templates[i][0]
            number = templates[i][1]
            tem = cv.imread(curretTemplate)
            if len(tem.shape) != 2:
                ...
                template = tem
            w, h = template.shape[::-1]
            result = cv.matchTemplate(src, template, cv.TM_CCOEFF_NORMED)
            loc = np.where(result >= 0.7)
            for pt in zip(*loc[::-1]): 
                topleft_x = pt[0]
                topleft_y = pt[1]
    ...
    for k in range(0, tem_Matching_Coordinates_Index):
        num = tem_Matching_Coordinates[k][0]
        if (num != 0) and (num != 1) and (num != 4):
            # print(" Has Only One Value  = False " + str(num))
            hasOnly_One_Value = False   

def identify_XAxis_UsingTempalte_Matching(): 
    global X_axis_cordinate, passfromTem_X, maxlength_X, pixcelForTicMark_Y
    found_X = False
    ...
    if y_count_max > 1:
        ...
        for h in range(0, noOfLines):
            len = X_arr[h][1]
            y = arr[h][1] 
            if (len == maxlength_X or  (len >= maxlength_X* 0.8)) :
                hasErrorin = True     
                if ( (y_coordnate_of_X_Axis <= y + (pixcelForTicMark_Y/2) + 5) and ( y_coordnate_of_X_Axis >= y-(pixcelForTicMark_Y/2) - 5) and ( y_coordnate_of_X_Axis != 0) ):    
                    X_axis_cordinate = h 
                    con = con + 1
        ...
        if count > 1:  
            if count == 2:
                if (X_arr[x1][0] < X_arr[x2][0]):
                    X_axis_cordinate = x1
                    return True
                elif (X_arr[x1][0] > X_arr[x2][0]):
                    X_axis_cordinate = x2
                    return True
            passfromTem_X = True
            found_X = find_X_Axis()
...
def identify_Y_AXis_UsingTempalte_Matching():
    global Y_axis_cordinate, passfromTem_Y, maxlength_Y, pixcelForTicMark_X, pixcelForTicMark_Y
    found_Y = False
        ...
        if count > 1:
            if count == 2: 
                ...
                for h in range(0, noOfLines):
                    ...
                    if l == maxlength_Y : 
                        samLine_Y = samLine_Y + 1 
                if ((Y_arr[y1][0]) < (Y_arr[y2][0])) and (samLine_Y != 1): 
                    Y_axis_cordinate = y2
                    return True
                elif ((Y_arr[y1][0]) > (Y_arr[y2][0])) and (samLine_Y != 1): 
                    Y_axis_cordinate = y1
                    return True 
            passfromTem_Y = True
            found_Y = find_Y_Axis() 
...
def seperateTemplateTo_X_Y_Axis():
    ...
    for i in range(0,30):
        ...
        if ( y >= (origin_Y - pixcelForTicMark_Y)) and ( y <= (origin_Y  + pixcelForTicMark_Y)): 
        ...
        if ( x >= (origin_X - pixcelForTicMark_X)) and ( x <= (origin_X + pixcelForTicMark_X)):
...
def identify_X_Axis_Ratio():
    global ratio_X_Axis_Value, ra_X_1, ra_X_2, hasTwoValuefor_XAxisRatio
    ...
    for i in range(0, 30):
        ...
        x = numberOf_Xaxis[i][1]
        if number != 0 and number != 1 and number != 4 :
            count = count + 1
        haveDuplicate = False
        if number != 0 and x != 0:
            for j in range (0,30):
    ...
    if count > 0:
        ...
        for i in range(0, 30):
            ...
            if number != 0 and x != 0 and pixcelForTicMark_X != 0:
                numberOf_TicMarks = int(round((x - origin_X )/pixcelForTicMark_X))
                if numberOf_TicMarks != 0:
                    ratio = round(number/numberOf_TicMarks,1)
                
                    if x_Axis_ratio_Index < 30:
                    
                        if (ratio < 0):
                            # print(" Ratio" + str(ratio))
                            x_Axis_ratio[x_Axis_ratio_Index] = int(ratio*(-1)*10)
                        else:
                            # print(" Ratio + " + str(ratio))
                            x_Axis_ratio[x_Axis_ratio_Index] = int(ratio*10)
                        x_Axis_ratio_Index = x_Axis_ratio_Index + 1
        ...
        if max_count >= 1 and value_2 == 0:
            ratio_X_Axis_Value = round(ratio_Of_XAxis/10,1)
        elif value_2 != 0 : 
            ra_X_1 = round(value_1/10,1)
            ra_X_2 = round(value_2/10,1)
            ratio_X_Axis_Value = ra_X_1
            hasTwoValuefor_XAxisRatio = True 

def identify_Y_Axis_Ratio():
    global ratio_Y_Axis_Value, ra_Y_1, ra_Y_2, hasTwoValuefor_YAxisRatio
    ...
    if count > 0:
        ...
        for i in range(0, 30):
            ...
            if number != 0 and pixcelForTicMark_Y != 0 and y !=0 :                                          #*******
                ...
                if numberOf_TicMarks != 0:
                    ...
                    if y_Axis_ratio_Index < 30:
                        if (ratio < 0):
                            y_Axis_ratio[y_Axis_ratio_Index] = int(ratio*(-1)*10)
                        else:
                            y_Axis_ratio[y_Axis_ratio_Index] = int(ratio*10)
                        y_Axis_ratio_Index = y_Axis_ratio_Index + 1 
        ...
        for i in range(0, 30):
            ...
            if (max_count == count): 
                if value_1 != val:
                    # print(" Ratio Y 1" + str(val))
                    value_2 = val

        if max_count >= 1 and value_2 == 0:
            ratio_Y_Axis_Value = round(ratio_Of_YAxis/10,1)

        elif value_2 != 0:
            ra_Y_1 = round(value_1/10,1)
            ra_Y_2 = round(value_2/10,1)
            # ratio_Y_Axis_Value = ra_Y_1
            hasTwoValuefor_YAxisRatio = True

def identify_X_Y():

    global X_axis_cordinate, found_X, Y_axis_cordinate, found_Y, arr_new, resized_img_New, Y_axis_count, X_axis_count, numberOf_Horizontal_new, numberOf_Vertical_new, numberOf_Graph_new

    resized_img_New = np.copy(resized_img)
    for x in range (0, width):
        for y in range (0 , height):
            resized_img_New[y,x] = 255
    ...
    for x in range (0, width):
        for y in range (0 , height):
            ...
            if val <= 128: 
                resized_img_New[y,x] = 1
    src_new = cv.cvtColor(resized_img_New, cv.COLOR_BGR2GRAY)
    dst2 = cv.Canny(src_new, 20, 200, None, 3)             
    linesP_new = cv.HoughLinesP(dst2, 1, np.pi / 180, 25, None, 4, 15) 
    noOfLines_new = 0
    if linesP_new is not None:
        noOfLines_new = len(linesP_new)
    
    reduce_2 = 0
    arr_new = [[0] * N for i in range(noOfLines_new)] 
    index = 1

    for i in range(0, noOfLines_new): 
        l = linesP_new[i][0] 
        #Store Values in a 2D array 
        if i == 0 : 
            arr_new[0][0] = l[0]
            ...
            for j in range(0,x): 
                if arr_new[j][0]-5 <= value1 and value1 <= arr_new[j][0]+5 and arr_new[j][1]-5 <= value2 and value2 <= arr_new[j][1]+5 and arr_new[j][2]-5 <= value3 and value3 <= arr_new[j][2]+5 and arr_new[j][3]-5 <= value4 and value4 <= arr_new[j][3]+5 : 
                    arr_new[j][0] = int(round((value1+arr_new[j][0])/2))
                    arr_new[j][1] = int(round((value2+arr_new[j][1])/2))
                    arr_new[j][2] = int(round((value3+arr_new[j][2])/2))
                    arr_new[j][3] = int(round((value4+arr_new[j][3])/2))
                    reduce_2 = reduce_2 +1
                    duplicate = 1
            
            if duplicate == 0: 
                arr_new[index][0] = value1
                arr_new[index][1] = value2
                arr_new[index][2] = value3
                arr_new[index][3] = value4
                index = index + 1
    
    X_arr_new = [[0] * 3 for i in range(noOfLines_new)]
    Y_arr_new = [[0] * 3 for i in range(noOfLines_new)]
    graphs_arr_new = [[0] * 4 for i in range(noOfLines_new)] 
    numberOf_Horizontal_new = 0
    numberOf_Vertical_new = 0
    numberOf_Graph_new = 0

    for k in range(0, noOfLines_new):  
        if not(arr_new[k][0] == 0 and arr_new[k][1] == 0 and arr_new[k][2] == 0 and arr_new[k][3] == 0) :
            x1 = arr_new[k][0]
            y1 = arr_new[k][1]
            x2 = arr_new[k][2]
            y2 = arr_new[k][3] 
            ...
            if (((y_difference >= -5) and (y_difference <= 5))  and (x_difference != 0)) :     
                X_arr_new[k][0] = y1
                X_arr_new[k][1] = lineLength
                X_arr_new[k][2] = k
                numberOf_Horizontal_new = numberOf_Horizontal_new + 1 
            # y axises 
            elif (((x_difference >= -5) and (x_difference <= 5)) and (y_difference != 0)) :    
                Y_arr_new[k][0] = x1
                Y_arr_new[k][1] = lineLength
                Y_arr_new[k][2] = k
                numberOf_Vertical_new = numberOf_Vertical_new + 1 
            # graph
            elif ((x_difference != 0) and (y_difference != 0)):
                m = y_difference/x_difference
                c = y1 - (x1*m) 
                graphs_arr_new[k][0] = m
                graphs_arr_new[k][1] = c
                graphs_arr_new[k][2] = lineLength
                graphs_arr_new[k][3] = k
                numberOf_Graph_new = numberOf_Graph_new + 1 

    maxlength_X = 0
    X_axis_count = 0
    Y_axis_cordinate_new = 0
    X_axis_cordinate_new = 0
    Y_axis_cordinate_new_1 = 0
    Y_axis_cordinate_new_2 = 0
    X_axis_cordinate_new_1 = 0
    X_axis_cordinate_new_2 = 0
    ...
    for h in range(0, noOfLines_new):  
        l = X_arr_new[h][1] 
        if l >= maxlength_X*0.8 : 
            X_axis_cordinate_new = h 
            # cv.line(cdstP_Linear, (arr_new[h][0], arr_new[h][1]), (arr_new[h][2], arr_new[h][3]), (50,0,255), 2, cv.LINE_AA)
            X_axis_count = X_axis_count + 1
            if X_axis_count == 1:
                X_axis_cordinate_new_1 = arr_new[h][1]
            elif X_axis_count == 2:
                X_axis_cordinate_new_2 = arr_new[h][1]
    
    if X_axis_count == 2:
        if (X_axis_cordinate_new_1 >= X_axis_cordinate_new_2 - 3 ) and (X_axis_cordinate_new_1 <= X_axis_cordinate_new_2 + 3 ):
            X_axis_count = 1

    # print ( " X_axis_count " + str(X_axis_count))
    # c_y = int(round((X_axis_cordinate_new_1 + X_axis_cordinate_new_2)/2))
    if X_axis_count == 1 :
        c_y = X_arr_new[X_axis_cordinate_new][0]
        for i in range (0, noOfLines):
            y = X_arr[i][0] 
            l = X_arr[i][1] 
            if l >= maxlength_X*0.8 :
                if (y >= c_y - 5) and (y <= c_y + 5 ):
                    X_axis_cordinate = i
                    found_X = True 
    ...
    for h in range(0, noOfLines_new):
        l = Y_arr_new[h][1] 
        if l >= maxlength_Y*0.8 : 
            Y_axis_cordinate_new = h 
            Y_axis_count = Y_axis_count + 1 
            if Y_axis_count == 1: 
                Y_axis_cordinate_new_1 = arr_new[h][0]
            elif Y_axis_count == 2: 
                Y_axis_cordinate_new_2 = arr_new[h][0]
    
    if Y_axis_count == 2:
        if (Y_axis_cordinate_new_1 >= Y_axis_cordinate_new_2 - 3 ) and (Y_axis_cordinate_new_1 <= Y_axis_cordinate_new_2 + 3 ):
            Y_axis_count = 1
    
    # print (" Y_axis_count  " + str(Y_axis_count))
    # c_x = int(round((Y_axis_cordinate_new_1 + Y_axis_cordinate_new_2)/2))
    if Y_axis_count == 1 : 
        c_x = Y_arr_new[Y_axis_cordinate_new][0]
        for i in range (0, noOfLines):
            x = Y_arr[i][0] 
            l = Y_arr[i][1] 
            if l >= maxlength_Y*0.8 :
                if (x >= c_x - 5) and (x <= c_x + 5 ):
                    Y_axis_cordinate = i
                    found_Y = True 
...
def main(argv):
    ...
    global cdstP, resized_img_2, src2, cdstP2, cdstP_X_Y, cdstP_Linear, resized_img_New, allLines, linesP, arr, X_arr, Y_arr, graphs_arr, noOfLines, origin_X, origin_Y, height, width, filename, src, srcTemplate, resized_img, ratio_X_Axis_Value, ratio_Y_Axis_Value, check_X, check_X_usingTemplateMatching, check_Y, check_Y_usingTemplateMatching, pixcelForTicMark_X, pixcelForTicMark_Y
   
    # Loads an image
    default_file = 'q2_2.png' 
    ...
    if(i_height <= 350 and i_width <= 350): 
        ...
        dim = (r_width, r_height)
        resized_img = cv.resize(img, dim, interpolation = cv.INTER_AREA) 
    resized_img_2 = np.copy(resized_img)
    ...
    if linesP is not None: # Check there are lines
        ...
        # 3 Store Lines Coordinate
        storeLineCoordinate()
            
        # 4 Separate X axis Y axis and Graphs
        separateX_Y_Graph()
        ...
        if found_X != True:
            ...
            # print(" Have we found x axis using text : " + str(check_X))
            # Identify X Axis Considering lenth If X Axis not found using text
            if(check_X != True) and (hasOnly_One_Value != True): 
        ...
        if found_Y != True:
            ...
            if(check_Y != True) and ((hasOnly_One_Value != True)): 
        ...
        # 10 identify X axis Ticmarks
        if (pixcelForTicMark_X == 0 ):
            identifyTicMarks_X_Axis() 

        # 11 identify Y axis Ticmarks
        if (pixcelForTicMark_Y == 0): 
            identifyTicMarks_Y_Axis() 

        # 12 Check graph is quadratic or Linear
        checkGraph()

        # 13 get the ratio of x and Y axis Values
        if numberOfCharactor > 0:
            identifyRatio()
 
        # 14 Seperate Templates into X, Y axis
        seperateTemplateTo_X_Y_Axis()

        # 15 Identify Ratio using Template matching
        ...
        if hasTwoValuefor_XAxisRatio == True:
            if (ra_X_1 == ratio_Y_Axis_Value) or (ra_X_2 == ratio_Y_Axis_Value):
                ratio_X_Axis_Value = ratio_Y_Axis_Value
        if hasTwoValuefor_YAxisRatio == True: 
            if (ra_Y_1 == ratio_X_Axis_Value) or (ra_Y_2 == ratio_X_Axis_Value):
                ratio_Y_Axis_Value = ratio_X_Axis_Value 


        
        # 16 if graph is in a grid Identify graph seperatly
        if ratio_X_Axis_Value < 1 and ratio_Y_Axis_Value < 1:
            dst = cv.Canny(MofologyImg_2, 20, 200, None, 3)  
            linesP = cv.HoughLinesP(dst, 1, np.pi / 180, 25, None, 1, 15)
            if linesP is not None: # Check there are lines
                noOfLines = len(linesP) 
                storeLineCoordinate() 
                ...
                checkGraph() 
        if ratio_X_Axis_Value < 1:
            new_pixcelForTicMark_X = int(round(pixcelForTicMark_X*(1/ratio_X_Axis_Value)))
            pixcelForTicMark_X = new_pixcelForTicMark_X
            ratio_X_Axis_Value = 1

        if ratio_Y_Axis_Value < 1:
            new_pixcelForTicMark_Y = int(round(pixcelForTicMark_Y*(1/ratio_Y_Axis_Value)))
            pixcelForTicMark_Y = new_pixcelForTicMark_Y
            ratio_Y_Axis_Value = 1 
        ratio_Y_Axis_Value = int(round(ratio_Y_Axis_Value))
        ratio_X_Axis_Value = int(round(ratio_X_Axis_Value))
        ...
        # if graph is a Quadratic Graph
            ...
            src3 = cv.cvtColor(resized_img_New, cv.COLOR_BGR2GRAY)
            dst2 = cv.Canny(src3, 20, 200, None, 3)             
            linesP = cv.HoughLinesP(dst2, 1, np.pi / 180, 25, None, 4, 15) 
            noOfLines = len(linesP)
            ...
            typeofGraph = None  
            if X_axis_count > 1:
                typeofGraph = "Parallel_X"
                print(" This lenear graph is Parallel to X Axis ")
            elif Y_axis_count > 1:
                print(" This lenear graph is Parallel to Y Axis")
                typeofGraph = "Parallel_Y"

            if typeofGraph == None:
                if numberOf_Horizontal_new > 1:
                    typeofGraph = "Parallel_X"
                    print(" This lenear graph Parallel to X Axis ")
                elif numberOf_Vertical_new > 1:
                    print(" This lenear graph Parallel to Y Axis")
                    typeofGraph = "Parallel_Y"

            
            if typeofGraph == "Parallel_X":  
                graph_Y_coordinate = 0 
                for i in range (0, noOfLines):
                    y = X_arr[i][0]
                    length = X_arr[i][1]
                    if ( length >= maxlength_X*0.5 and y != 0)  and not(y <= origin_Y + 3 and y >= origin_Y - 3 ): 
                        graph_Y_coordinate = y
                        cv.line(cdstP_Linear, (arr[i][0], arr[i][1]), (arr[i][2], arr[i][3]), (50,0,255), 2, cv.LINE_AA)
                num = int(round((origin_Y - graph_Y_coordinate)/pixcelForTicMark_Y))
                print(" Equation : y = " + str(num * ratio_Y_Axis_Value))

            elif typeofGraph == "Parallel_Y": 
                graph_X_coordinate = 0
                for i in range (0, noOfLines):
                    x = Y_arr[i][0]
                    length = Y_arr[i][1]
                    if ( length >= maxlength_X*0.5 and x != 0) and not(x <= origin_X + 3 and x >= origin_X - 3 ): 
                        graph_X_coordinate = x
                        cv.line(cdstP_Linear, (arr[i][0], arr[i][1]), (arr[i][2], arr[i][3]), (50,0,255), 2, cv.LINE_AA)
                num = int(round((graph_X_coordinate - origin_X)/pixcelForTicMark_X))
                print(" Equation : x = " + str(num * ratio_X_Axis_Value))
 
    # cv.imshow("Source", src) 
    cv.imshow("Input Image ", resized_img_2)    
    # cv.imshow("Source Template", srcTemplate)  
    # cv.imshow("Source", src2) 
    #cv.imshow("Source Template", temSrc)
    cv.imshow("X Y Axis Origin and Tic Marks", cdstP_X_Y)    
    if ( graphType == "linear" or graphType == "Parallel"): 
        cv.imshow("Text Detection", cdstP) 
        cv.imshow("Linear graph", cdstP_Linear) 
        if graphType == "Parallel":
            cv.imshow("X axis Y axis and Graph ", resized_img_New)
        # cv.imshow("Mofology ", MofologyImg_2) 
    elif (graphType == "Quadratic"): 
        cv.imshow("Mofology ", MofologyImg_2) 
        cv.imshow("Min Max points and Graph Points ", cdstP2)
        cv.imshow(" Text Detection", cdstP) 
        cv.imshow("Graph Lines", cdstP_Linear) 
