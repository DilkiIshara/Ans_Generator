...
src = src2 = MofologyImg = graphType = srcTemplate = temSrc = cdstP_X_Y = None
...
def identifyNumbersRelated_X_Y_Axis():
    ...
    if numberOfCharactor > 0:
        ...
        # for i in range(0, len(textCoordinate)):
            ...
            if(((textCoordinate[i][1]) <= (origin_X+pixcelForTicMark_X)) and ((textCoordinate[i][1]) >= (origin_X-pixcelForTicMark_X))):
                ...
                if (character_Y_Cordinate > origin_Y and pixcelForTicMark_Y != 0 ):
                    numberOf_Tic_Mark_Y_Axis = round((character_Y_Cordinate - origin_Y)/ pixcelForTicMark_Y)

                elif (character_Y_Cordinate < origin_Y and (pixcelForTicMark_Y != 0)):
                ...
                if(character_X_Cordinate > origin_X and (pixcelForTicMark_X != 0)):
                    numberOf_Tic_Mark_X_Axis = round((character_X_Cordinate - origin_X)/ pixcelForTicMark_X)

                elif (character_X_Cordinate < origin_X and (pixcelForTicMark_X != 0)):
    ...
    if (total_ratio_X_Axis_Value > 0 and (ratio_X_Axis_Index > 0)):
        ratio_X_Axis_Value = round(total_ratio_X_Axis_Value/(ratio_X_Axis_Index))
        #### print("Ratio_X_Axis_Value " + str(ratio_X_Axis_Value))

    if (total_ratio_Y_Axis_Value > 0 and (ratio_Y_Axis_Index > 0)):
...
def templateMatching():
    global srcTemplate, numberOf_Xaxis, numberOf_Yaxis, tem_y_arr, tem_x_arr, tem_Matching_Coordinates, xIndex , yIndex, temSrc
    temSrc = src
    ...
    templates[14][1] = 5
    templates[15][0] = "Ex5.png"
    templates[15][1] = 5
    templates[16][0] = "Ey10.png"
    templates[16][1] = 10
    templates[17][0] = "Ey5.png"
    templates[17][1] = 5
    templates[18][0] = "Ey5.2.png"
    templates[18][1] = 5
    templates[19][0] = "Ey10.png"
    templates[19][1] = 10
...
def identify_XAxis_UsingTempalte_Matching():
    ...
    #     x_cor = tem_x_arr[i][0]
    #     for j in range (0, 20):
    #         current_x_cor = tem_x_arr[j][0]
    #         if ( (current_x_cor <= (x_cor +3 )) and (current_x_cor >= (x_cor - 3 ))):
    #             count = count +1
    #     tem_x_arr[i][1] = count
    ...
    for i in range(0, 20):
        ...
        if y_cor != 0:
            for j in range (0, 20):
                current_y_cor = tem_y_arr[j][0]
                if ( (current_y_cor <= (y_cor +4 )) and (current_y_cor >= (y_cor - 4 ))):
                    count = count +1
        tem_y_arr[i][1] = count
        # print (" coordinate " + str(y_cor) + " count" + str(count))

    # find max count
    # x_count_max = 
    y_count_max = 0
    # x_coordnate_of_Y_Axis = 0
    ...
    for i in range (0,20): 
        current_y_count_max = tem_y_arr[i][1]
        if (current_y_count_max == y_count_max) or (current_y_count_max == (y_count_max - 1)):
            total_y_coordnate_of_X_Axis = total_y_coordnate_of_X_Axis + tem_y_arr[i][0]
            count = count + 1
    ...
    if y_count_max > 1:
        ...
        for h in range(0, noOfLines):
            len = X_arr[h][1]
            y = arr[h][1] 
            if (len == maxlength_X) or (len >= maxlength_X - (maxlength_X * 0.2)):
                # print("  ---------------   " + str(y))
                # print ("      ======================  " + str(len))     
                if ( (y_coordnate_of_X_Axis <= y + 15) and ( y_coordnate_of_X_Axis >= y-15) and ( y_coordnate_of_X_Axis != 0) ):    
                    X_axis_cordinate = h 
                    found_X = True
                    return found_X 
...
def identify_Y_AXis_UsingTempalte_Matching():
    ...
    for i in range(0, 20):
        ...
        if x_cor != 0: 
            for j in range (0, 20):
                current_x_cor = tem_x_arr[j][0]
                if ( (current_x_cor <= (x_cor +4 )) and (current_x_cor >= (x_cor - 4 ))):
                    count = count +1
        tem_x_arr[i][1] = count
        # print (" x_coordnate_of_Y_Axis " + str(x_cor) + " count " + str(count))
    ...
    #     for j in range (0, 20):
    #         current_y_cor = tem_y_arr[j][0]
    #         if ( (current_y_cor <= (y_cor +3 )) and (current_y_cor >= (y_cor - 3 ))):
    #             count = count +1
    #     tem_y_arr[i][1] = count
    ...
    for i in range (0,20):
        current_x_count_max = tem_x_arr[i][1]
    ...
    for i in range (0,20):
        current_x_count_max = tem_x_arr[i][1]
        if (current_x_count_max == x_count_max) or (current_x_count_max == (x_count_max-1)) :
            total_x_coordnate_of_Y_Axis = total_x_coordnate_of_Y_Axis + tem_x_arr[i][0]
            count = count + 1
    ...
    if x_count_max > 1:
        ...
        for h in range(0, noOfLines):
            ...
            if (len == maxlength_Y) or (len >= maxlength_Y - (maxlength_Y * 0.3)): 
                ...
                if ( (x_coordnate_of_Y_Axis <= x + 15) and ( x_coordnate_of_Y_Axis >= x-15) and (x_coordnate_of_Y_Axis != 0)):     
...
def main(argv):
    ...
    global cdstP, src2, cdstP2, cdstP_X_Y, cdstP_Linear, allLines, linesP, arr, X_arr, Y_arr, graphs_arr, noOfLines, origin_X, origin_Y, height, width, filename, src, srcTemplate
